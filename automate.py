...
# Bloqueio estrito de notícias não-tecnológicas e esportes
TERMOS_BLOQUEADOS = [
    ...
    "placar", "jogo de hoje", "arbitragem", "cazétv", "paulistão", "copa do brasil"
]
...
def selecionar_foto_hd(titulo, categoria):
    ...
    if any(k in t for k in ["memória", "memoria", "ram", "ssd", "hbm", "cxl", "armazenamento", "sk hynix"]):
        return "https://images.unsplash.com/photo-1591799264318-7e6ef8ddb7ea?w=1200&auto=format&fit=crop&q=80"
    elif any(k in t for k in ["celular", "smartphone", "smartphones", "galaxy", "dobrável", "pixel"]):
        return "https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=1200&auto=format&fit=crop&q=80"
    elif any(k in t for k in ["whatsapp", "zap", "golpe", "invasão", "hack", "segurança", "vulnerabilidade"]):
    ...
    elif any(k in t for k in ["apple", "mac", "macos", "macbook", "iphone", "ios", "ipad"]):
...
def generate_bilingual_post(news_item):
    ...
    if not data:
        ...
        data = {
            ...
            "content_en": f"## Overview\n\n{news_item['summary']}\n\n### Strategic & Engineering Analysis\n\nThis breakthrough in {news_item['category']} represents a pivotal architectural shift. By advancing hardware efficiency, low-latency protocols, and scalable compute frameworks, this announcement establishes new operational benchmarks for developers, enterprises, and infrastructure operators worldwide.\n\n### Key Takeaways\n\n1. Enhanced throughput and architectural optimization.\n2. Reduced operational expenditure and token overhead.\n3. Strategic realignments across the global technology ecosystem.\n\n*Read original coverage at: [{news_item['link']}]({news_item['link']})*",
            "title_pt": news_item["title"],
            "content_pt": f"## Visão Geral do Acontecimento\n\n{news_item['summary']}\n\n### Análise Estratégica e de Engenharia\n\nEste avanço no segmento de {news_item['category']} representa uma transformação arquitetural de grande impacto. Ao elevar os padrões de eficiência de hardware, protocolos de baixa latência e orquestração escalável, este desenvolvimento redefine as regras de concorrência e infraestrutura no mercado global.\n\n### Principais Lições e Próximos Passos\n\n1. Otimização de vazão de dados e redução de gargalos de processamento.\n2. Controle rigoroso de custos operacionais e consumo de computação.\n3. Reconfiguração competitiva entre grandes players e ecossistemas abertos.\n\n*Acompanhe a matéria original em: [{news_item['link']}]({news_item['link']})*"
        }
    return data
...
def save_posts(data, all_posts_manifest, news_item, idx):
    ...
    title_pt = data.get("title_pt") or news_item["title"]
    title_en = data.get("title_en") or news_item["title"]
    
    desc_pt_raw = data.get("content_pt") or news_item["summary"]
    desc_en_raw = data.get("content_en") or news_item["summary"]
    
    desc_pt = re.sub(r'[#*_`]', '', desc_pt_raw).strip()[:200] + "..."
    desc_en = re.sub(r'[#*_`]', '', desc_en_raw).strip()[:200] + "..."

    foto_hd = selecionar_foto_hd(title_pt, category)
    ...
    print(f"📁 [{idx+1}] Matéria Bilíngue Salva: {title_pt}")
...
if __name__ == "__main__":
    ...
    print(f"🎉 Finalizado com sucesso! {len(all_posts_manifest)} matérias bilíngues salvas.")
